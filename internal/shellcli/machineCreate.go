...
func createMachine(configurationPath string) {
	...
	// TODO :
	//   - implements feature where an ssh key is generated by default for each vm
	//		- Test the freyja config method 'Audit'
	//		- implement the method to build the ssh keys with provision files
	//      - IMPORTANT inject the key path  in the freyja config for users that needs it !
	//     		The key path is in user home's .ssh dir.
	//    		The path in the config is in fact a symlink to the actual key in the freyja machine's config dir.
	//		- create the ssh key for the machine's default user in the freyja machine dir
	//		- if the users are defined without ssh keys, create an ssh key for each user in the freyja machine dir
	//		- create a symbolic link for each key in the /home/$USER/.ssh dir
	//		- inject each key's symbolic link in the freyja configuration
	//   - implement the command 'freyja machine ssh -n vm1' to avoid 'freyja machine info -n vm1' to get ip beforehand
